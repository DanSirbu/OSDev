...
//Some bit manipulation is done so that non-aligned (i.e. not 4 byte offsets) still work
//Size is the number of bytes to write
//Ex. offset = 0x2, size = 2, write bytes 0x2, 0x3, does not work for more than 4 bytes unaligned, ex. offset = 0x3, size = 2 is undefined 
...
void pci_write_field(uint32_t dev, uint8_t field, uint8_t size, uint32_t value_to_write) {
    ...
    if(bytes_offset == 0) {
        ...
        tempValue &= (~bit_masks[size - 1] << (bytes_offset * 8)) | bit_masks[bytes_offset - 1]; //Zero out the bytes we want to set
    }

    tempValue |= (value_to_write & bit_masks[size - 1]) << (bytes_offset * 8); //Set the bytes
    
    //kpanic_fmt("Writing 0x%x at 0x%x\n", tempValue, aligned_addr_to_read_write);
...
void pci_find_devices() {
    ...
    uint16_t vendor = pciConfigReadWord(bus, slot, 0, 0);
        if(vendor != NON_EXISTANT_DEVICE) {
            uint16_t device_id = pciConfigReadWord(bus, slot, 0, 2); //High word of offset 0

            dev.device_full_id = (vendor << 16) | device_id;
            dev.device_pci_address = convertToPCIAddress(bus, slot, 0);
