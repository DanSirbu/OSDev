...
static unsigned long long my_intscan(SCANF_STATE *f, unsigned base, int pok,
...
static int my_vfscanf(SCANF_STATE *restrict f, const char *restrict fmt,
		      va_list ap)
{
	int width;
	int size;
		...
		case '[':
			...
			if (size == SIZE_l) {
				...
				while (scanset[(c = shgetc(f)) + 1]) {
					assert_msg(
						1 == 2,
						"scanf: wchar32_t not implemented\n",
						"");
